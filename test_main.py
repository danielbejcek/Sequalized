...
class TestEmployeeDatabase(unittest.TestCase):
    def setUp(self):
        ...
        self.test_emp2 = EmployeeDatabase("Steve","Jackson",10,"M",10)
        ...
        self.last_row_id,self.added_employee = self.test_emp1.add_employee()
    ...
    def test_database_employee_insert(self):
        ...
        get_db_employee = self.test_emp1.get_employee(self.last_row_id)
    ...
    def test_add_employee_character_input(self):
        ...
        """Check to see if age or salary are not a string and that both are above 0."""
        if isinstance(self.test_emp2.age, int) and self.test_emp2.age > 0 and isinstance(self.test_emp2.salary, int) and self.test_emp2.salary > 0:
            has_string = False
        else:
            has_string = True

        self.assertFalse(has_string, "age or salary cannot be string and must be above 0")
